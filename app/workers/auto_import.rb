      	...
      	chunk.each do |h|
	      	...
	      	unless h["rec_type"].nil? || h["rec_type"] == ""
					...
					unless h["student_id"].nil?
				        ...
				        unless images.any?
				          	image = package.student_images.new(:load_id => h["load_id"],:student_id => student.id, :image_file_name => h["url"], :folder => h["folder"], :grade => h["grade"], :url => h["url"], :shoob_id => h["shoob_id"])
